<template>
  <!-- 包裹级联选择器的容器，限制高度，并设置overflow: hidden; 高度使用数据的方式动态修改 -->
  <div class="h-selector-wrapper" :style="{height: parentHeight+'px'}">
    <!-- 级联选择器的真实节点，无序列表 ref属性用来获取真实的DOM节点 style通过数据去设置样式 监听需要用到的事件 touchstart 触摸开始 touchmove 触摸移动 touchend 触摸结束 transitionend 过渡结束 -->
    <ul
      ref="wrapper"
      :style="style"
    ...
    >
      ...
      <li class="h-selector-option"></li>
      <li class="h-selector-option">请选择</li>
...
<script>
export default {
  name: "hWrapper", // 接收prop数据，表示需要操作父组件中value对象上的某个指定数据
  props: { prop: { type: String, required: true } }, // 定义初始化数据
  data() {
    return {
      // 初始化样式
      style: {
  ...
  watch: {
    ...
    propValue(newValue) {
      ...
      this.$emit("change", newValue); // 重置当前级联选择器的位置
      this.formatAddress();
    }
  }, // 实例挂载完成
  mounted() {
  ...
  methods: {
    ...
    formatData() {
      ...
      let children = this.$children; // 保存当前选项的长度
      this.optionLength = children.length; // try {
    ...
    formatAddress() {
      // 保存当前组件的子组件
      let children = this.$children; // 查询到当前组件中与外层传入的数据对应的子组件的索引值
      let index = children.findIndex(
        item => item.value === this.$parent.value[this.prop]
      ); // 将索引值赋予activeIndex，如果能查找到对应的，说明外部传值了，
      // 就自动滑动到当前索引的位置，否则就显示请选择
      this.activeIndex = index > -1 ? index : -1;
      // 通过索引值与当前option的高度计算距离，并执行动画
      this.style.transform = `translate3d(0px,${-this.activeIndex *
    ...
    touchMove(e) {
      // 保存当前移动的位置
      let moveY = e.changedTouches[0].pageY; // 保存当前移动的方向，往下拉的话，moveY - this.startY为正，往上拉的话为负
      this.direction = moveY - this.startY; // 设置拖拽移动
      this.style.transform = `translate3d(0px,${this.prevY +
        this.direction}px,0px)`;
    }, // 触碰结束
    touchEnd(e) {
      // 设置过渡动画
      this.style.transition = "transform .4s"; // 保存结束位置
      this.endY = e.changedTouches[0].pageY; // 保存结束时间
      this.endTime = e.timeStamp; // 保存上一次移动的距离
      this.prevY = this.style.transform.split(",")[1].slice(0, -2) * 1; // 计算当前移动到的位置索引
      let activeIndex = -Math.round(this.prevY / this.optionHeight); // 计算当前手指从触碰开始到结束移动的距离
      let distance = Math.abs(this.endY - this.startY); // 计算当前手指从触碰开始到结束的时间差
      let interval = this.endTime - this.startTime; // 根据方向不同来计算应该移动到对应的索引值上
      // 大于0 为向下拉
      if (this.direction > 0) {
        // 通过距离与时间差来计算最新的坐标索引
